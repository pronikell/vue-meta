...
class JsdomWebpage extends webpage.Webpage {
  async open(url, readyCondition = 'body') {
    ...
    const onJsdomError = (err) => {
      throw new index.BrowserError(this, err)
    };
    ...
    if (readyCondition) {
      ...
      await new Promise((resolve, reject) => {
        ...
        async function waitForElement() {
          ...
          if (iter > 100) {
            reject(new index.BrowserError(t, `Timeout reached on waiting for readyCondition: ${readyCondition}`));
            return
          }
...
class JsdomBrowser extends index.Browser {
  constructor(config = {}) {
    config.xvfb = false;
  ...
  _page(url, readyCondition) {
    const page = new JsdomWebpage(this);
    return page.open(url, readyCondition)
  }
}

exports.default = JsdomBrowser;
